...
Tensor LayerMaxPool2D::forward(const Tensor& input){
    _last_batch = input.shape[0];
    size_t out_H = _shape_output[1];
    size_t out_W = _shape_output[2];

    Tensor output(Shape({_last_batch, _channels, out_H, out_W}), false);
    _mask = Tensor(input.shape, false);// pour replacer la val lors du back
...
Tensor LayerMaxPool2D::backward(const Tensor& grad){
    size_t out_H = _shape_output[1];
    size_t out_W = _shape_output[2];
    Tensor grad_out(Shape({_last_batch,_channels,_shape_input[1],_shape_input[2]}), false);
                ...
                for(size_t x = 0; x < out_W; x++){
                    ...
                    for(size_t dec_y = 0; dec_y < 2; dec_y++){
                        for(size_t dec_x = 0; dec_x < 2; dec_x++){
                            ...
                            if(_mask(b,c,yy,xx) == 1.0f){
                                grad_out(b,c,yy,xx) = grad(b,c,y,x);
                            }
                        }
                    }
                }
