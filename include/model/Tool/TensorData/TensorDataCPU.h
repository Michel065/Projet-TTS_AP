...
public:
    ...
    void init(Shape shape, bool alea, int val_init) override;
    ...
    Tensor matmul(Shape shape_a,const Tensor& b) const override;
    ...
    Tensor extraction_section_axe_0(Shape shape_a, int debut, int fin) const override;

    //methode autre
    bool equal(const Tensor& b) const override;
    std::vector<size_t> recalul_shape() const override;
